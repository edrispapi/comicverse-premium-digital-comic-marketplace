        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12">
            ...
            <div className="space-y-6">
              <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.1 }} className="text-4xl md:text-5xl font-bold tracking-tight">
                {comic.title}
              </motion.h1>
              ...
              <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.4 }} className="text-lg text-neutral-300 max-w-prose">
                {comic.description}
              </motion.p>
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.5 }} className="flex items-center flex-wrap gap-4 pt-4">
                ...
                <Button size="lg" variant="outline" className="border-2" onClick={() => toggleWishlist(comic)}>
                  <Heart className={cn("mr-2 h-5 w-5", isInWishlist && "fill-red-500 text-red-500")} /> Wishlist
                </Button>
                {comic.previewImageUrls.length > 0 && (
                  <Sheet>
                    ...
                    <SheetContent className="w-full sm:max-w-3xl bg-comic-card border-l-white/10 text-white">
                      <SheetHeader>
                        <SheetTitle>Preview: {comic.title}</SheetTitle>
                      </SheetHeader>
                      <div className="py-4">
                        <Carousel>
                          <CarouselContent>
                            {comic.previewImageUrls.map((url, index) => (
                              <CarouselItem key={index}>
                                <img src={url} alt={`Preview page ${index + 1}`} className="w-full h-auto object-contain rounded-md" />
                              </CarouselItem>
                            ))}
                          </CarouselContent>
