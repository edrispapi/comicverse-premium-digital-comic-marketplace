...
import { Book, Clock, DollarSign, Download, Activity, BarChart, PieChart, TrendingUp } from 'lucide-react';
import { ResponsiveContainer, LineChart, Line, XAxis, YAxis, CartesianGrid, Tooltip, Legend, Pie, Cell, Bar } from 'recharts';
        ...
        <motion.div variants={containerVariants} initial="hidden" animate="visible">
          ...
          <motion.div variants={itemVariants} className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
            <Card className="bg-comic-card border-white/10">
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2"><CardTitle className="text-sm font-medium">Comics Read</CardTitle><Book className="h-4 w-4 text-muted-foreground" /></CardHeader>
              <CardContent>{statsLoading ? <Skeleton className="h-8 w-1/2" /> : <div className="text-2xl font-bold">{stats?.reads}</div>}</CardContent>
            </Card>
            <Card className="bg-comic-card border-white/10">
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2"><CardTitle className="text-sm font-medium">Hours Listened</CardTitle><Clock className="h-4 w-4 text-muted-foreground" /></CardHeader>
              <CardContent>{statsLoading ? <Skeleton className="h-8 w-1/2" /> : <div className="text-2xl font-bold">{stats?.hours}</div>}</CardContent>
            </Card>
            <Card className="bg-comic-card border-white/10">
              ...
              <CardContent>{statsLoading ? <Skeleton className="h-8 w-1/2" /> : <div className="text-2xl font-bold">${stats?.spent.toFixed(2)}</div>}</CardContent>
            </Card>
          </motion.div>
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <CardContent className="h-80">
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={mockStreakData} margin={{ top: 5, right: 20, left: -10, bottom: 5 }}>
                      ...
                      <Legend />
                      <Line type="monotone" dataKey="pages" stroke="#EF4444" strokeWidth={2} dot={{ r: 4, fill: '#EF4444' }} activeDot={{ r: 8 }} />
                    </LineChart>
                  </ResponsiveContainer>
                </CardContent>
              </Card>
            ...
            <motion.div variants={itemVariants}>
              <Card className="bg-comic-card border-white/10"><CardHeader><CardTitle className="flex items-center gap-2"><PieChart /> Genre Breakdown</CardTitle></CardHeader>
                <CardContent className="h-80">
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
            ...
            <motion.div variants={itemVariants} className="lg:col-span-2">
              <Card className="bg-comic-card border-white/10"><CardHeader><CardTitle className="flex items-center gap-2"><BarChart /> Top Authors by Rating</CardTitle></CardHeader>
                <CardContent className="h-80">
                  <ResponsiveContainer width="100%" height="100%">
          ...
          <motion.div variants={itemVariants} className="mt-12 text-right">
            <Button variant="outline" onClick={() => toast.info("Exporting your data...")}><Download className="mr-2 h-4 w-4" /> Export Data (CSV)</Button>
          </motion.div>
        </motion.div>
      </main>
      <Footer />
