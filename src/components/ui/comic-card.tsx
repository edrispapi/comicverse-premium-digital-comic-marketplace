  ...
  return (
    <div className={cn("flex items-center gap-0.5", className)}>
      {[...Array(fullStars)].map((_, i) => (
        <Star key={`full-${i}`} className="w-4 h-4 text-red-400 fill-red-400" />
      ))}
      {halfStar && <StarHalf key="half" className="w-4 h-4 text-red-400 fill-red-400" />}
      {[...Array(emptyStars)].map((_, i) => (
        <Star key={`empty-${i}`} className="w-4 h-4 text-red-400/60" />
      ))}
    </div>
  );
}
export function ComicCard({ comic }: ComicCardProps) {
  ...
  return (
    <HoverCard openDelay={200} closeDelay={100}>
      <HoverCardTrigger asChild>
        <Link to={`/comic/${comic.id}`} className="block group">
          <motion.div
            ...
            whileHover={{ y: -8, scale: 1.02, boxShadow: '0 10px 20px rgba(239, 68, 68, 0.2)' }}
          ...
          >
            ...
            <div className="p-3 md:p-4">
              <h3 className="font-bold text-base md:text-lg truncate text-white group-hover:text-red-400 transition-colors">{comic.title}</h3>
              {firstAuthor && (
                <div className="mt-1 text-xs md:text-sm text-neutral-400 truncate">
                  <Link to={`/authors/${firstAuthor.id}`} className="hover:text-red-400 transition-colors">
                    {firstAuthor.name}
                  </Link>
                </div>
              )}
              <div className="flex items-center justify-between mt-2 text-sm">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-neutral-400">({comic.ratings?.votes || 0})</span>
                </div>
      ...
      <HoverCardContent className="w-80 glass-dark border-white/10 text-white p-0" side="right" align="start">
        <div className="p-4 space-y-3">
          <h3 className="font-bold text-lg">{comic.title}</h3>
          <p className="text-sm text-neutral-300 line-clamp-4">{comic.description}</p>
          <div className="flex items-center justify-between pt-2 border-t border-white/10">
            <div className="flex items-center gap-2">
              <RatingStars rating={ratingAvg} />
              <span className="font-bold text-red-400">${comic.price.toFixed(2)}</span>
            </div>
            <div className="flex items-center gap-2">
              <Button
                size="icon"
                ...
                onClick={(e) => handleInteraction(e, () => toggleWishlist(comic))}
              >
                <Heart className={cn("w-4 h-4 text-white", isInWishlist && "fill-red-500 text-red-500")} />
              </Button>
              <Button
                size="icon"
